...
contract openHashClaim {
    ...
    event SeedProvided(uint256 indexed gameId, address indexed reporter, bytes32 seed);
    ...
    function provideSeed(uint256 gameId, bytes32 seed) external {
        HashGame storage h = hashGame[gameId];
        if (!h.active) revert InvalidInput("not active");
        if (h.finished) revert InvalidInput("finished");
        if (h.claimer == address(0)) revert InvalidInput("not claimed");
        if (msg.sender != h.reporter) revert InvalidInput("not reporter");
        if (h.seed != bytes32(0)) revert InvalidInput("seed already provided");
        if (seed == bytes32(0)) revert InvalidInput("seed zero");

        uint256 currentTime = h.timeType ? block.timestamp : block.number;
        h.reportTimestamp = uint48(currentTime);
        h.seed = seed;

        emit SeedProvided(gameId, h.reporter, seed);
    }

    function disproveReporter(uint256 gameId) external {
        HashGame storage h = hashGame[gameId];
        if (!h.active) revert InvalidInput("not active");
        if (h.finished) revert InvalidInput("finished");
        if (h.claimer == address(0)) revert InvalidInput("not claimed");
        if (h.seed != bytes32(0)) revert InvalidInput("seed provided");

        uint256 currentTime = h.timeType ? block.timestamp : block.number;
        if (currentTime <= h.settlementTime + h.reportTimestamp) revert InvalidInput("reveal window not over");

        address payable claimer = payable(h.claimer);
        ...
        remainder += uint256(oldReward) * (h.multiplier - 100) / 100; // refund claimer's wager
    ...
    function disproveClaim(uint256 gameId) external {
        ...
        if (h.finished) revert InvalidInput("finished");
        if (h.claimer == address(0)) revert InvalidInput("not claimed");
        if (h.seed == bytes32(0)) revert InvalidInput("seed not provided");
    ...
    function breakReporter(uint256 gameId, uint256 nonce) external {
        ...
        if (!h.active) revert InvalidInput("not active");
        if (h.finished) revert InvalidInput("finished");
        if (h.claimer != msg.sender) revert InvalidInput("not claimer");
        if (h.seed == bytes32(0)) revert InvalidInput("seed not provided");
        ...
        if (uint256(hash) > uint256(h.threshold)) {
            ...
            uint256 protocolFee = h.protocolFee * remainder / 1e7;
            remainder = remainder - protocolFee;
            remainder += uint256(oldReward) * (h.multiplier - 100) / 100; // refund wager (caller is always the claimer)
